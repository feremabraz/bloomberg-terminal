    ...
    <div className="p-4" style={{ backgroundColor: colors.background, color: colors.text }}>
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex items-center gap-4">
          <div>
            <label htmlFor="region-select" className="block text-xs mb-1">Region</label>
            <Select 
              value={selectedRegion} 
              onValueChange={(value) => setSelectedRegion(value as "americas" | "emea" | "asiaPacific")}
            >
              <SelectTrigger 
                id="region-select"
          ...
          <div>
            <label htmlFor="security-select" className="block text-xs mb-1">Security</label>
            <Select 
          ...
          <div>
            <label htmlFor="benchmark-select" className="block text-xs mb-1">Benchmark</label>
            <Select 
      ...
      >
                ...
                <table className="w-full text-xs">
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td style={{ color: selectedSecurityData.change >= 0 ? colors.positive : colors.negative }}>
                        {selectedSecurityData.change.toFixed(2)} ({selectedSecurityData.pctChange.toFixed(2)}%)
                      </td>
                    </tr>
                    <tr>
              ...
              {benchmarkData && (
                <table className="w-full text-xs">
                  <tbody>
                    ...
                    <tr>
                      <td className="py-1 font-medium">Change:</td>
                      <td style={{ color: benchmarkData.change >= 0 ? colors.positive : colors.negative }}>
        ...
        <TabsContent value="table" className="space-y-4">
          <div className="p-4 border rounded-sm" style={{ borderColor: colors.border, backgroundColor: colors.surface }}>
            ...
            <p className="text-xs mb-4">Historical RMI values comparing {selectedSecurityData?.id} to {benchmarkData?.id}</p>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                ...
                {Array.from({ length: 10 }).map((_, i) => {
                  ...
                  const secValue = selectedSecurityData ? (selectedSecurityData.value * (1 - i * 0.005)).toFixed(2) : "0.00";
                  const benchValue = benchmarkData ? (benchmarkData.value * (1 - i * 0.003)).toFixed(2) : "0.00";
                  const rmi = (100 * (1 + i * 0.002)).toFixed(2);
                  const change = (i === 0 ? 0 : (Number.parseFloat(rmi) - Number.parseFloat((100 * (1 + (i-1) * 0.002)).toFixed(2)))).toFixed(2);
                  ...
                  return (
                    <tr key={uniqueKey} className="border-b" style={{ borderColor: colors.border }}>
                      ...
                      <td className="py-2 text-right" style={{ color: isPositive ? colors.positive : colors.negative }}>
                        {isPositive ? "+" : ""}{change}
                      </td>
                    </tr>
                  );
        ...
        <TabsContent value="analysis" className="space-y-4">
          <div className="p-4 border rounded-sm" style={{ borderColor: colors.border, backgroundColor: colors.surface }}>
            ...
            <p className="text-xs mb-4">
              The Relative Market Index (RMI) measures the performance of {selectedSecurityData?.id} relative to {benchmarkData?.id}.
            </p>
            
            <h4 className="text-xs font-bold mt-4 mb-2">Key Insights</h4>
            <ul className="list-disc pl-5 text-xs space-y-2">
              <li>
                Current RMI value is <span className="font-bold">{selectedSecurityData?.rmi}</span>, indicating 
              ...
              <li>
                {selectedSecurityData?.id} has {Math.abs(selectedSecurityData?.pctChange || 0) > Math.abs(benchmarkData?.pctChange || 0)
                  ? "higher"
                  : "lower"} volatility than the benchmark index.
              </li>
              <li>
                Year-to-date performance shows {selectedSecurityData?.ytd || 0 > benchmarkData?.ytd || 0
                  ? "stronger returns"
                  : "weaker returns"} compared to the benchmark.
              </li>
            </ul>
            ...
            <p className="text-xs">
              Based on the current RMI trend, consider {Number.parseFloat(selectedSecurityData?.rmi || "100") > 100 
